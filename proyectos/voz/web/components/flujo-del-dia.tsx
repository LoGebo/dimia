...
export async function FlujoDelDia({
  ...
  const enlace = (cambios: Record<string, string | undefined>) => {
    ...
    const todos: Record<string, string | undefined> = { ...extra, recurso: parametros.recurso, filtro: parametros.filtro, orden: parametros.orden, ...cambios };
    ...
    <>
      ...
      <TiraIndicadores>
        <Cifra
          ...
          pildora={variacion === null ? undefined : `${variacion >= 0 ? "+" : ""}${variacion}% vs. hace 7 días`}
        ...
        <Cifra
          ...
          glifo={Glifos.reloj}
          pildora={esperando.length === 0 ? "al día" : `${minutosLegibles(Math.max(...esperando.map((r) => minutosDesde(r.inicio, ahora))))} la que más`}
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <Chip activo={!recursoActivo} href={enlace({ recurso: undefined })}>Todo</Chip>
        {activos.map((r) => (
        ...
        {FILTROS.filter((f) => conteoFiltro[f.valor] > 0 || filtro === f.valor).map((f) => (
          <Chip key={f.valor} activo={filtro === f.valor} href={enlace({ filtro: filtro === f.valor ? undefined : f.valor })} conteo={conteoFiltro[f.valor]}>
            {f.nombre}
          </Chip>
        ))}
        <Chip activo={orden === "espera"} href={enlace({ orden: orden === "espera" ? undefined : "espera" })}>Por espera</Chip>
        <span className="numeros ml-auto font-mono text-[11px] text-tinta-3">{visibles.length} de {delDia.length}</span>
      </div>

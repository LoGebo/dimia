...
const ESTADOS: Record<EstadoPedido, { nombre: string; tono: TonoOperacion; late: boolean }> = {
...
export function TableroPedidos({ pedidos, zona, cobrados = new Map() }: { pedidos: Pedido[]; zona: string; cobrados?: Map<string, string> }) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
      {pedidos.map((p, i) => (
        <TarjetaPedido key={p.id} pedido={p} zona={zona} ahora={ahora} cobrado={cobrados.get(p.id) ?? null} retraso={i} />
      ))}
    </div>
  );
}
 ...
 */
function TarjetaPedido({ pedido, zona, ahora, cobrado, retraso }: { pedido: Pedido; zona: string; ahora: number; cobrado: string | null; retraso: number }) {
  ...
  return (
    <article
      style={{ animationDelay: `${Math.min(retraso, 8) * 40}ms` }}
      className={`kit-revela flex flex-col border bg-panel transition-colors duration-150 ${
        nuevo ? "border-acento" : pedido.estado === "abierto" ? "border-alerta/50" : "border-linea hover:border-linea-fuerte"
    ...
    >
      <header className="flex items-start justify-between gap-3 border-b border-linea px-4 py-3">
        <div className="min-w-0">
          <p className="numeros font-mono text-[30px] leading-none font-medium tracking-[0.12em] text-tinta">{pedido.codigo}</p>
        ...
        <div className="flex shrink-0 flex-col items-end gap-1.5 pt-1">
          ...
          {nuevo ? (
            <span className="numeros bg-acento px-1.5 py-0.5 font-mono text-[10px] tracking-[0.18em] text-acento-tinta uppercase">Nuevo</span>
          ) : null}
        </div>
      </header>

      <div className="border-b border-linea px-4 py-2.5">
        <p className="truncate text-[17px] font-semibold text-tinta">{pedido.cliente_nombre ?? "Sin nombre todavía"}</p>
        <p className="numeros font-mono text-[13px] text-tinta-2">{telefono(pedido.telefono)}</p>
        <p className="mt-1.5 flex items-center gap-2 text-[15px] font-medium text-tinta">
          <i aria-hidden="true" className={`h-1.5 w-1.5 flex-none ${pedido.tipo === "domicilio" ? "bg-acento" : "bg-tinta-3"}`} />
          {TIPOS[pedido.tipo]}
        </p>
        {pedido.tipo === "domicilio" ? (
          <p className={`mt-0.5 pl-3.5 text-[15px] ${pedido.direccion ? "text-tinta-2" : "text-alerta"}`}>{pedido.direccion ?? "Falta la dirección"}</p>
        ) : null}
      </div>

      <div className="flex items-center justify-between border-b border-linea px-4 py-1.5">
        <span className="etiqueta">Lo que pidió</span>
      ...
      <ul className="flex-1 divide-y divide-linea">
        {pedido.items.map((item, i) => (
          <li key={`${item.nombre}-${i}`} className="flex items-baseline gap-3 px-4 py-2">
            <span className="numeros w-9 shrink-0 font-mono text-[22px] leading-none font-medium text-tinta">{item.cantidad}×</span>
            <span className="min-w-0 flex-1">
              <span className="block text-[17px] leading-tight text-tinta">{item.nombre}</span>
              {item.notas ? <span className="mt-0.5 block border-l-2 border-alerta pl-2 text-[14px] leading-tight font-medium text-alerta">{item.notas}</span> : null}
            </span>
            <span className="numeros shrink-0 font-mono text-[14px] text-tinta-2">{moneda(item.subtotal)}</span>
          </li>
        ))}
      ...
      {pedido.notas ? <p className="border-t border-linea px-4 py-2 text-[14px] leading-snug text-tinta-2">{pedido.notas}</p> : null}
      ...
      <div className="flex items-center justify-between gap-3 border-t border-linea bg-panel-2 px-4 py-3">
        ...
        <span className={`numeros font-mono text-[26px] leading-none font-medium tracking-[-0.02em] ${cobrado ? "text-bueno" : "text-tinta"}`}>

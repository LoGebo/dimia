...
export async function dondeAgente(agenteId: string, donde: "dimia" | "local"): Promise<EstadoLocal & { error?: string }> {
  const r = await orquestador(`/agentes/${agenteId}/donde`, { method: "POST", body: JSON.stringify({ donde }) });

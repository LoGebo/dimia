...
export default async function Pedidos({
  ...
  const filtro =
    FILTROS.find((f) => f.valor === parametros.estado)?.valor ?? "pendientes";
  ...
  const cobrados = new Map(
    pagos
      .filter((p) => p.estado === "pagado" && p.pedido_id)
      .map((p) => [p.pedido_id!, p.monto]),
  );
  ...
  const filtrar = (valor: string) =>
    valor === "todos"
      ...
      : valor === "pendientes"
        ? todos.filter(
            (p) => p.estado === "abierto" || p.estado === "confirmado",
          )
  ...
  return (
    <>
      ...
      <Encabezado
        ...
        acciones={
          <NavegarDia
            anterior={enlace({ dia: sumarDias(dia, -1) })}
            hoy={enlace({ dia: hoy })}
            siguiente={enlace({ dia: sumarDias(dia, 1) })}
          />
        }
      />
      ...
      <div className="space-y-4 px-5 py-5">
        <TiraIndicadores>
          ...
          <Cifra
            ...
            tono={
              resumen.abiertos + resumen.confirmados > 0 ? "alerta" : "bueno"
            }
          />
          <Cifra
            etiqueta="Vendido"
            valor={moneda(resumen.vendido)}
            numero={resumen.vendido}
            formato="moneda"
            pildora="confirmados y entregados"
          />
          <Cifra
            etiqueta="Ticket promedio"
            valor={moneda(resumen.ticket)}
            numero={resumen.ticket ?? undefined}
            formato="moneda"
            pildora="por pedido cerrado"
          />
        </TiraIndicadores>

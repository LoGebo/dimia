...
# en el llavero dimia-firma y el perfil «Dimia App Store» (ver asc.py).
set -e
cd "$(dirname $0)/.."
...
# Llavero propio para firmar (su contraseña en ~/.appstoreconnect/firma-llavero): el de sesión pide aprobar
# con un diálogo y, sin nadie frente a la pantalla, codesign falla con errSecInternalComponent.
LLAVERO=~/Library/Keychains/dimia-firma.keychain-db
security unlock-keychain -p "$(cat ~/.appstoreconnect/firma-llavero)" $LLAVERO
codesign --force --timestamp --keychain $LLAVERO --sign "Apple Distribution" --entitlements $T/ent.plist --requirements firma.req $A
